...
export const Hero: React.FC<HeroProps> = ({ onSeeWorksClick, onReachOutClick }) => {
  ...
  return (
    <section id="home" className="relative z-10 w-full h-[100svh] min-h-[100svh] flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* Hero Content */}
      <div className="relative z-20 text-center flex flex-col items-center justify-center px-4 max-w-4xl mt-8 sm:mt-12 select-none w-full">
        
        {/* Eyebrow */}
        <span className="blur-in text-[10px] sm:text-xs text-muted uppercase tracking-[0.3em] mb-4 sm:mb-8 font-medium">
          Ruang Pilihan '26
        </span>

        {/* Name */}
        <h1 className="name-reveal text-4xl sm:text-6xl md:text-8xl lg:text-9xl font-display italic leading-[0.95] tracking-tight text-text-primary mb-4 sm:mb-6 max-w-full break-words">
          Highlanderstay
        </h1>

        {/* Role line */}
        <div className="blur-in text-base sm:text-xl lg:text-2xl text-text-primary/90 font-light mb-3 sm:mb-4 px-2">
          Temukan{' '}
          <span 
            key={roleIndex} 
        ...
        {/* Description */}
        <p className="blur-in text-xs sm:text-sm md:text-base text-muted max-w-md mb-8 sm:mb-12 font-light leading-relaxed px-2">
        ...
        {/* CTA Buttons */}
        <div className="blur-in flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3 sm:gap-4 w-full sm:w-auto px-4 sm:px-0">
          {/* Browse Spaces */}
          <button 
            onClick={onSeeWorksClick}
            className="relative group rounded-full text-xs sm:text-sm font-semibold px-6 sm:px-7 py-3.5 min-h-[44px] bg-text-primary text-bg border border-transparent transition-all duration-300 hover:scale-105 hover:bg-bg hover:text-text-primary flex items-center justify-center text-center shadow-lg"
          >
          ...
          <button 
            ...
            className="relative group rounded-full text-xs sm:text-sm font-semibold px-6 sm:px-7 py-3.5 min-h-[44px] border-2 border-stroke bg-bg text-text-primary transition-all duration-300 hover:scale-105 hover:border-transparent flex items-center justify-center text-center"
          >
            {/* Background Hover Border Ring */}
            <span className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10 p-[1.5px] accent-gradient" style={{ margin: '-1.5px' }} />
            Jadwalkan Tur
      ...
      {/* Scroll Indicator */}
      <div className="absolute bottom-4 sm:bottom-8 left-1/2 transform -translate-x-1/2 hidden xs:flex sm:flex flex-col items-center gap-2 pointer-events-none">
